...
Create a function that puts a given tab in the failed state and silently blocks any further updates.
...
parameters:
*   **target** : *[object Tab]*  
    the tab that will be put in the failed state using the arguments of the new function, and that will be silently blocking any further updates, when that new function is being called.

returns:
*   **newFunction** : *function*  
    a newly constructed function that puts a given tab in the failed state, silently blocking any further updates.
