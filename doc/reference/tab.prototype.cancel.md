...
effect:
*   any further updates for this tab ([tab.return()][ref-tab.prototype.return], [tab.throw()][ref-tab.prototype.throw] or [tab.settle()][ref-tab.prototype.settle]) will throw an error.
*   any further subscriptions for notifications from this tab will throw an error.
...
try { Tab.construct().return().cancel().valueOf(); } catch (e) { e.message === "cancelled" }
Tab.construct().settle().cancel().valueOf() === undefined
try { Tab.construct().throw().cancel().valueOf(); } catch (e) { e.message === "cancelled" }
````
