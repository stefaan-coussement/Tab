...
---
### Tab.doCaptureWith( target ) » newFunction

core principle:

````
Tab.doCaptureWith(target).call(subject, ...arguments)
~
target.return(subject, ...arguments);
````

### Tab.doCaptureWith( target, processor ) » newFunction

core principle:

````
Tab.doCaptureWith(target, processor).call(subject, ...arguments)
~
target.return(subject, ...arguments);
...
````javascript
function doCaptureWith(processor) {
