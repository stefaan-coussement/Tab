...
Create a function that uses a given tab to store another function's result, using the new function's subject as a first argument for the other function.

<br />

---
### Tab.deferWith( target ) » newFunction

parameters:
*   **target** : *[object Tab]*  
    the tab that will get updated with the subject and arguments of the new function, when the latter is being called.

returns:
*   **newFunction** : *function*  
    a newly constructed function that uses a given tab to store this new function's subject and arguments.

core principle:

````
Tab.deferWith(target).call(subject, ...arguments) 
~ 
...
````
Tab.deferWith(target, processor).call(subject, ...arguments)
~
try {
    target.return(processor.call(target, subject, ...arguments));
}
catch (e) {
    target.throw(e);
}
