...
---
### Tab.trace( target ) » newFunction

core principle:

````
Tab.trace(target).call(subject, ...arguments)
~
try {
...
````
Tab.trace(target, processor).call(subject, ...arguments)
~
try {
    target.return(subject, ...arguments, 
...
function trace(message, processor) {
    ...
    tracer = function () {
        try {
                if (message == null) {
                    target.return(subject, ...arguments, "returned", 
                ...
                if (message == null) {
                    target.return(subject, ...arguments, "trown", e);
                }
                else {
